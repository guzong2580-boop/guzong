...
export default function App() {
  ...
  const [eName,setEName]=useState(""); const [eDate,setEDate]=useState("");
  const [eTime,setETime]=useState(""); const [ePlace,setEPlace]=useState("");
  const [eStatus,setEStatus]=useState("upcoming");
  ...
  function openExamForm(exam=null) {
    setEditExam(exam);
    setEName(exam?.name||""); setEDate(exam?.date||""); setETime(exam?.time||"");
  ...
  function saveExam() {
    ...
    if (editExam) setExams(exams.map(e=>e.id===editExam.id?{...e,name:eName,date:eDate,time:eTime,place:ePlace,status:eStatus}:e));
    else setExams([...exams,{id:Date.now(),name:eName,date:eDate,time:eTime,place:ePlace,status:eStatus}]);
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
            <div style={{ background:GRAD, borderRadius:16, padding:"20px 20px", color:"white", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
              <div>
                ...
                <div style={{ fontSize:11, opacity:0.75 }}>{nextExam?.date||""}</div>
              </div>
